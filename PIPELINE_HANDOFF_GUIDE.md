...
> **Pipeline Components**: `real_image_pipeline/` $\rightarrow$ `outputs/` $\rightarrow$ `TrynTest/gnn_outfit_compatibility_v2.py`
...
3. **Graph Construction**: Passes these embeddings as **Node Features** ($\mathbf{x}_i \in \mathbb{R}^{512}$) into the **Outfit Graph Neural Network (GNN)** to predict outfit compatibility, style harmony, and fill-in-the-blank (FITB) recommendations.
...
| **Vector Embeddings for GNN** | Embeddings computed on noisy raw boxes had background room features polluting the vector space. | Faded saliency cutouts produced degraded embeddings. | **Pure Garment Embeddings**: CLIP encodes pure fabric on neutral canvas $\rightarrow$ **512-D unit vectors ($\|v\|_2 = 1.0$)**. |

---

## 🏗️ 3. Pipeline Architecture (Step-by-Step)

The production pipeline is located in [`real_image_pipeline/`](file:///c:/Users/Suprateek%20Yawagal/OneDrive/Documents/AI-based-personal-stylist/real_image_pipeline) and consists of three core stages:
...
- **Input**: Raw RGB user photo ($H \times W \times 3$)
...
- **Embedding Generation**: Encodes the isolated crop composited on a neutral canvas to produce:
  $$\mathbf{e} = \text{CLIP}_{\text{vision}}(\text{garment\_crop}) \in \mathbb{R}^{512}$$
  $$\mathbf{x} = \frac{\mathbf{e}}{\|\mathbf{e}\|_2}, \quad \|\mathbf{x}\|_2 = 1.0$$
...
The downstream **Outfit Compatibility GNN** models an outfit as a **heterogeneous garment graph** $\mathcal{G} = (\mathcal{V}, \mathcal{E})$:

```mermaid
graph TD
    subgraph Outfit Graph G
        Top["👕 Top Node (x_top ∈ R^512)"]
        Bottom["👖 Bottom Node (x_bottom ∈ R^512)"]
        Shoes["👟 Shoe Node (x_shoe ∈ R^512)"]
        Acc["🕶️ Accessory Node (x_acc ∈ R^512)"]
        
        Top ===|"Edge w = 1.0 (Top ↔ Bottom)"| Bottom
        Top ---|"Edge w = 0.5 (Top ↔ Shoe)"| Shoes
        Bottom ===|"Edge w = 0.8 (Bottom ↔ Shoe)"| Shoes
        Top -.-|"Edge w = 0.4 (Top ↔ Acc)"| Acc
        Bottom -.-|"Edge w = 0.4 (Bottom ↔ Acc)"| Acc
        Shoes -.-|"Edge w = 0.3 (Shoe ↔ Acc)"| Acc
    end
    
    Outfit Graph G --> WeightedGNNLayer["Weighted GNN Layers (Message Passing)"]
    WeightedGNNLayer --> Readout["Graph Pooling / Readout"]
    Readout --> Score["Outfit Compatibility Score s ∈ [0, 1]"]
```

### 1. Node Features ($\mathbf{X} \in \mathbb{R}^{N \times 512}$)
Each node $i \in \mathcal{V}$ corresponds to an extracted garment (`.png`) and its initial feature representation is its **CLIP embedding**:
$$\mathbf{x}_i = \text{np.load}(\text{f"outputs/\{outfit\_id\}/\{file\}.npy"})$$

### 2. Edge Index & Category Weighting
Edges represent compatibility relationships between clothing items. The GNN in [`TrynTest/gnn_outfit_compatibility_v2.py`](file:///c:/Users/Suprateek%20Yawagal/OneDrive/Documents/AI-based-personal-stylist/TrynTest/gnn_outfit_compatibility_v2.py) uses fashion-domain edge importance:
